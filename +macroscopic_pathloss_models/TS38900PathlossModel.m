classdef TS38900PathlossModel < macroscopic_pathloss_models.generalPathlossModel
    % 通过38900计算路径损耗（传播损耗）
    properties
        frequency     % 系统频率
        environment   % 场景
    ...
    methods
        function obj = TS38900PathlossModel(frequency,environment,SYS_config) %初始化
            ...
            for i = 1:row*col
                if d_2d_in(i) == 0
                    if d_2d(i)<=18
                        ...
                        g(i) = (1.25*exp(1)-6)*(d_2d(i)^3)*exp(-d_2d(i)/150);
                    end
                else
                    if d_2d_out(i)<=18
                        g(i) = 0;
                    else
                        g(i) = (1.25*exp(1)-6)*(d_2d_out(i)^3)*exp(-d_2d_out(i)/150);
                    end
                end
            end
            [m,n] = find(h_ut(:,:)>=13);
            ...
            for i=1:row*col
                if d_bp_1(i)<10
                    ...
                    if d_2d_out(i)<d_bp_1(i)
                        pl(i) = 32.4 + 20*log10(d_3d_out(i)) + 20*log10( obj.frequency/1e9);
                    else
                        pl(i)=32.4 + 40*log10(d_3d_out(i)) + 20*log10( obj.frequency/1e9)-10*log10(d_bp_1(i)^2+(obj.h_bs-h_ut(i))^2);
                    end
                end
            end
            obj.PL_UMa_LOS = pl;
        ...
        function [pl,comp]=pathloss_urban_macro_LOS_NLOS(obj,d_2d,h_ut,d_2d_in)
            ...
            LOS_posibility=min(18./d_2d_out,1).*(1-exp(-d_2d_out/63))+exp(-d_2d_out/63).*(1+C);%LOS概率
            r_rnd=rand(row,col);
            compare=r_rnd>LOS_posibility;%如果compare=1则采用NLOS，如果compare=0，采用LOS;
            [NLOS_x,NLOS_y]=find(compare==1);
            NLOS_length=length(NLOS_x);
        ...
        function [pl,comp]=pathloss_urban_micro_LOS_NLOS(obj,d_2d,h_ut,d_2d_in)
            ...
            LOS_posibility=min(18./d_2d_out,1).*(1-exp(-d_2d_out/36))+exp(-d_2d_out/36);%LOS概率
            r_rnd=rand(row,col);
            compare=r_rnd>LOS_posibility;%如果compare=1则采用NLOS，如果compare=0，采用LOS;
            [NLOS_x,NLOS_y]=find(compare==1);
            NLOS_length=length(NLOS_x);
