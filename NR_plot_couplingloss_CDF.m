...
BS_element_gain = -pathloss_BS_antenna_array_gain+pathloss-2; %统计被干扰链路的基站天线增益，UMA-2dB UMI-4，后续还需要调整
...
UE_element_gain = -pathloss_UE_antenna_array_gain+pathloss-14;%%统计被干扰链路的用户天线增益，-22dB，后续还需要调整
...
fprintf(fid,'%f\n',R1);
fprintf(fid,'该文档记录统计到的宏观路损（传输损耗+穿透损耗）\n');
fclose(fid);
...
fprintf(fid,'%f\n',R2);
fprintf(fid,'该文档记录统计到的路损+BS端天线单元增益\n');
fclose(fid);
...
fprintf(fid,'%f\n',R3);
fprintf(fid,'该文档记录统计到的路损+BS端天线阵列增益\n');
fclose(fid);
...
fprintf(fid,'%f\n',R4);
fprintf(fid,'该文档记录统计到的路损+UE端天线单元增益\n');
fclose(fid);
...
fprintf(fid,'%f\n',R5);
fprintf(fid,'该文档记录统计到的路损+UE端天线阵列增益\n');
fclose(fid);


BS_element_gain = sort(BS_element_gain);
index = 1:length(BS_element_gain)/100:length(BS_element_gain);
index = round(index);
R7 = BS_element_gain(index);
R7 = [R7 max(BS_element_gain)];
fid=fopen('.\calibration\R7.txt','wt');
fprintf(fid,'%f\n',R7);
fprintf(fid,'该文档记录统计BS端的单元增益\n');
fclose(fid);



UE_element_gain = sort(UE_element_gain);
index = 1:length(UE_element_gain)/100:length(UE_element_gain);
index = round(index);
R8 = UE_element_gain(index);
R8 = [R8 max(UE_element_gain)];
fid=fopen('.\calibration\R8.txt','wt');
fprintf(fid,'%f\n',R8);
fprintf(fid,'该文档记录统计UE端的单元增益\n');
fclose(fid);
